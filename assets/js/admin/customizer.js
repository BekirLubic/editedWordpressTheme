...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.bonbeno webkitAnimationEnd.bonbeno', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.bonbeno webkitAnimationEnd.bonbeno' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.bonbeno webkitAnimationEnd.bonbeno', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.bonbeno webkitAnimationEnd.bonbeno' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.bonbeno', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.bonbeno' );
			} );
		},
